...
router.patch("/", authMiddleware, async (request, response) => {
  ...
  const { job_id, status, applicant_id } = request.body
  try {
    if (is_employer) {
      // Job must belong to Employer
      if (!applicant_id || !job_id || !status)
        return response.status(400).json({ error: "A field has been omitted." })
      const jobExists = await Job.findOne({ _id: job_id })
      if (!jobExists)
        return response
          .status(400)
          .json({ error: "Job for this application does not exist." })
      if (String(jobExists.creator_id) !== _id)
        return response.status(400).json({
          error: "Permission denied. Application does not belong to you.",
        })
    } else {
      // Applicant must exist for Seeker
      if (!job_id || !status)
        return response.status(400).json({ error: "A field has been omitted." })
      const applicationExist = await JobApplication.findOne({
        job_id,
        applicant_id: _id,
      })
      if (!applicationExist)
        return response
          .status(400)
          .json({ error: "Application does not exist or belong to you." })
    }
  } catch (e) {
    return response
      .status(400)
  ...
  const update = { status }
  const query = is_employer
    ? { applicant_id, job_id }
    : { applicant_id: _id, job_id }
...
router.post("/", authMiddleware, async (request, response) => {
  ...
  if (is_employer)
    return response
      .status(400)
      .json({ error: "You need to be logged in as a job seeker to apply." })
  const { job_id, user_message } = request.body

  try {
    let jobExists = await Job.findOne({ _id: job_id })
    if (!jobExists)
      return response.status(400).json({ error: "Job does not exist" })
    if (!jobExists.open)
      return response
        .status(400)
        .json({ error: "Job has been closed, you cannot apply to this job." })
    let applicationExists = await JobApplication.findOne({
      applicant_id: _id,
      job_id,
    })
  ...
  application
    ...
    .then((result) =>
      response
        .status(200)
        .json({ message: "You have successfully applied for this job." })
    )
    .catch((error) => response.status(400).json({ error }))
})
...
router.get("/list", authMiddleware, (request, response) => {
  ...
  JobApplication.paginate(query, options)
    ...
    .catch((error) =>
      response.status(400).json({ error: "Failed to load applications." })
    )
})
...
router.get("/list/employer", authMiddleware, async (request, response) => {
  ...
  if (!job_id)
    return response.status(400).json({ error: "Job_id was not supplied." })
  try {
    let jobExists = await Job.findOne({ _id: job_id })
    if (!jobExists)
      return response.status(400).json({ error: "This job does not exist" })
  } catch {
